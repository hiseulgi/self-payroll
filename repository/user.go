...
func (p *userRepository) FindByID(ctx context.Context, id int) (*model.User, error) {
	...
	// TODO: DONE.
	// buat fungsi untuk mencari user berdasarkan ID pada parameter
	user := new(model.User)

	if err := p.Cfg.Database().WithContext(ctx).Preload("Position").Where("id = ?", id).First(user).Error; err != nil {
		return nil, err
	}
...
func (p *userRepository) Create(ctx context.Context, user *model.User) (*model.User, error) {
	// TODO: DONE.
	// buat fungsi untuk membuat user berdasarkan struct parameter
...
func (p *userRepository) UpdateByID(ctx context.Context, id int, user *model.User) (*model.User, error) {
	// TODO: DONE.
	// buat fungsi untuk update user berdasarkan struct parameter
	if err := p.Cfg.Database().WithContext(ctx).Model(&model.User{ID: id}).Updates(user).Find(user).Error; err != nil {
		return nil, err
	}
